...
function buscarLibros($biblioteca, $termino) {
    $termino = mb_strtolower($termino); // normalizar a minúsculas
    return array_filter($biblioteca, function($libro) use ($termino) {
        return (mb_strpos(mb_strtolower($libro['titulo']), $termino) !== false) ||
               (mb_strpos(mb_strtolower($libro['autor']), $termino) !== false);
    });
}
...
function generarReporteBiblioteca($biblioteca) {
    ...
    arsort($autores);
    $autorMasLibros = key($autores);

    return [
        "total_libros" => $totalLibros,
        "libros_prestados" => $prestados,
        "libros_disponibles" => $totalLibros - $prestados,
