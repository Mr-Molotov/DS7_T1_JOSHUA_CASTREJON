...
// 9. Función para contar ciudades por inicial
function contarCiudadesPorInicial($arr, $letra) {
    $contador = 0;
    $letra = mb_strtoupper($letra); // Asegurar mayúsculas y soporte UTF-8
    foreach ($arr as $ciudad) {
        if (mb_strtoupper(mb_substr($ciudad, 0, 1)) === $letra) {
            $contador++;
        }
    }
    return $contador;
...
// 10. Ejemplo de uso
echo "\nNúmero de ciudades que comienzan con 'S': " . contarCiudadesPorInicial($ciudades, 'S') . "\n";
echo "Número de ciudades que comienzan con 'N': " . contarCiudadesPorInicial($ciudades, 'N') . "\n";
